...
public class Register 
{
	...
	public static void onItemRegister(RegistryEvent.Register<Item> event)
	{
		...
		if(Config.Tinker)
			ModItems.castliquidstar.addParts();
	}
	...
	public static void onBlockRegister(RegistryEvent.Register<Block> event)
	{
		...
		if(Config.WirelessMover)
			GameRegistry.registerTileEntity(TileMover.class, ModBlocks.Mover.getRegistryName().toString());
	}
	...
	public static void registerItemColourHandlers(final ColorHandlerEvent.Item event) 
	{
		...
		if(Config.Tinker)
		{
			if(Config.Harmonious && Config.UpgradeArmor)
			{
				itemColors.registerItemColorHandler(new IItemColor() 
				{
					@Override
					public int colorMultiplier(ItemStack stack, int tintIndex) 
					{
						return tintIndex > 0 ? -1 : ((ItemUpgradeArmor)stack.getItem()).getColor(stack);
					}
				}, ModItems.UpgradeHelmet, ModItems.UpgradeChestplate, ModItems.UpgradeLeggings, ModItems.UpgradeBoots);
			}
		}
